//app.discount.$id.tsx  Update Discounts
import type { ActionFunctionArgs, LoaderFunctionArgs } from "@remix-run/node";
import { useActionData, useLoaderData, useNavigation } from "@remix-run/react";
import { Page } from "@shopify/polaris";
import { Collection, DiscountClass, Product } from "app/types/admin.types";
...
export const action = async ({ params, request }: ActionFunctionArgs) => {
  ...
  if (method === DiscountMethod.Code) {
    ...
    //  Fetch enriched product/collection info 
  const products = await getProductsByIds(request, parsedConfiguration.productIds);
  const collections = await getCollectionsByIds(request, parsedConfiguration.collectionIds);

  // Extract numeric discount ID
  const discountNodeId = id.split("/").pop();
  if (!discountNodeId) {
  ...
  await prisma.discount.update({
    ...
    data: {
      ...
      productPercentage: parsedConfiguration.cartLinePercentage || 0,
      orderPercentage: parsedConfiguration.orderPercentage || 0,
      deliveryPercentage: parsedConfiguration.deliveryPercentage || 0,
...
export const loader = async ({ params, request }: LoaderFunctionArgs) => {
  ...
  const products = discount?.configuration?.productIds
    ? await getProductsByIds(
        request,
        discount.configuration.productIds.map((id: string) =>
          id.startsWith("gid://") ? id : `gid://shopify/Product/${id}`,
        ),
    )
    : [];

  return { discount, collections, products };
};

export default function VolumeEdit() {
  const actionData = useActionData<ActionData>();
  const { discount: rawDiscount, collections, products } = useLoaderData<LoaderData>();
  ...
  return (
    <Page>
      <ui-title-bar title={`Edit the ${rawDiscount.title}`}>
